...
- The separate local Playwright smoke targets the selected worktree's
  loopback-only Compose project. Each project has isolated PostgreSQL and
  Redis state; a validated PostgreSQL snapshot is restored explicitly when
  realistic local data is required. It is development evidence and never
  replaces deterministic CI or deployed release evidence.
