...
usage() {
  cat <<'EOF'
Usage:
  ./scripts/workspace-dev.sh up                start full stack
  ./scripts/workspace-dev.sh down              stop full stack (including orphans)
  ./scripts/workspace-dev.sh restart           down + up
  ./scripts/workspace-dev.sh status            quick status table
  ./scripts/workspace-dev.sh doctor            full diagnostic
  ./scripts/workspace-dev.sh logs [svc]        tail combined or single service log
  ./scripts/workspace-dev.sh check             validate prerequisites
  ./scripts/workspace-dev.sh kill-orphans      kill anything bound to our ports

Database (local Postgres in docker):
  ./scripts/workspace-dev.sh db-backup         dump local DB to backups/<ts>.sql.gz (custom format + gzip)
  ./scripts/workspace-dev.sh db-backups        list existing backups (newest first)
  ./scripts/workspace-dev.sh db-restore FILE   restore a backup into the local DB (drops + recreates)
  ./scripts/workspace-dev.sh db-shell          open psql against the local DB
