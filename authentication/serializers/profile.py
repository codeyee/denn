...
class ProfileSerializer(serializers.ModelSerializer):
    ...
    def get_ratings(self, obj):
        user_ratings = Rating.objects.filter(user=obj).select_related(
            'content_item').order_by('-created_at')

        return RatingSerializer(user_ratings, many=True, context=self.context).data

    def get_lists_count(self, obj):
        return UserList.objects.filter(Q(owner=obj) | Q(members=obj)).distinct().count()
