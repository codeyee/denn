...
const handleListHover = (listId: number) => {
    ...
    if (typeof window !== "undefined") {
        queryClient.prefetchQuery(["list", listId], () =>
            ...
            <div key={list.id}>
                ...
                {isInList && (
                    <span className="text-xs text-gray-400">
                        Already added{" "}
                        {formatDate(
                            existingLists.in_lists.find((l) => l.id === list.id)
                                .added_at
                        )}
                    </span>
                )}
...
export const PLATFORM_ICONS: Record<string, string> = {
    ...
    "disney-plus":
        "https://images.justwatch.com/icon/147638/s100/disney-plus.webp",
...
**Feature:**
Fetch paginated items for immediate display, but also fetch next page and full list in background for faster navigation and innmediate action in reorder mode.
...
// Background: Fetch page 2 and all
useEffect(() => {
    const { items: page2Items } = useListItems(listId, {
        page: 2,
        page_size: 20,
    });

    const fetchAll = async () => {
        const allItems = await api.get(
            `/api/lists/${listId}/items/?page_size=0`
        );
