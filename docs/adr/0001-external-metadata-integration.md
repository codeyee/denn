...
El problema que el Sprint 06 pidió cerrar: la topología nunca fue una decisión, fue una acumulación. Documentación contradice código (`core/README.md` describe `core` como un "centralized API gateway", lo cual no se sostiene). Y existen dos clientes lógicos contra `proxy` (uno desde `web`, otro desde `core`) sin un contrato común.
...
- [`core/README.md`](../../core/README.md) se reescribe en PR-6D para no afirmar que `core` es el gateway.
